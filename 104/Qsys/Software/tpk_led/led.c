...
alt_u32 ice_callback(void* context) {
    ice_flag = true;
	return PERIOD_ICE;
}

alt_u32 rgb_callback(void* context) {
    rgb_flag = true;
	return PERIOD_RGB;
}
...
//                                     7); //启动定时器允许中断，连续计数
    static alt_alarm ice_alarm, rgb_alarm;
    alt_alarm_start(&ice_alarm, PERIOD_ICE, ice_callback, NULL);
    alt_alarm_start(&rgb_alarm, PERIOD_RGB, rgb_callback, NULL);
}
